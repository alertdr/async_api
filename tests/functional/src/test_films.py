...
async def test_get_film_list_with_sort(fill_db_movies, make_get_request, reverse: bool):
...
async def test_get_film_list_with_sort_and_pagination(fill_db_movies, make_get_request,
                                                      page_number: int, reverse: bool):
    page_size = 1
    response = await make_get_request(f'/films?sort={"-" if reverse else ""}imdb_rating&'
...
async def test_get_film_list_filtered(fill_db_movies, make_get_request, genre, count):
...
async def test_get_film(fill_db_movies, make_get_request, uuid):
...
async def test_wrong_parameters(fill_db_movies, make_get_request, page_number, size):
    response = await make_get_request(f'/films?page[number]={page_number}&page[size]={size}')
    assert response.status == 422


@pytest.mark.asyncio
async def test_wrong_film_id(fill_db_movies, make_get_request):
